...
make_option("-p", "--poll", dest="p"),
    ...
    def export_by_group(self, poll):
        ...
        for group in groups:
            ...
            responses = Response.objects.filter(poll=poll.pk).filter(contact__groups = group.pk)
            response_data_list = []
            excel_file_path = os.path.join(os.path.join(settings.MEDIA_ROOT, 'downloads'), 'poll_%d_%s.xlsx' % (poll.pk,group.name))
