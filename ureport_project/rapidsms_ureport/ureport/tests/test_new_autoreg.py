...
from rapidsms.tests.harness.base import MockBackend
...
class NewRapidTest(MockBackend, TestCase):
...
#        self.kibenga_colline = Location.objects.create(type=colline, name='kibenga')
    ...
    def testJoinResponse(self):
        """Outbox should have a response to the join mesage"""

        self.receive('join', identity='1112223333')
        self.assertEqual(self.outbox[0].text, settings.OPT_IN_CONFIRMATION['en'])
